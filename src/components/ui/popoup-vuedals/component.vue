...
<template>
  <transition tag="div" name="vuedal">
    <div class="vuedals" v-show="vuedals.length" tabindex="0" @keyup.esc.prevent="handleEscapeKey($event)">
      <!-- @click="dismiss() if we want close by shadow -->
      <div class="vuedal modal uui-modal fade" id="popup1" tabindex="-1" role="dialog"
           aria-hidden="true" v-for="(vuedal, index) in vuedals" :key="index" :class="getCssClasses(index)" @click.stop>
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header" v-if="(vuedal.title || vuedal.dismissable) && !vuedal.header">
              <h4 class="modal-title">{{ vuedal.title }}</h4>
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true" @click="dismiss()" v-if="vuedal.dismissable" ><span></span><span></span></button>
            </div>
            ...
            <div class="modal-footer">
              <epam-button class="large" @click="dismiss()">Cancel</epam-button>
              <epam-button class="large" markup="lime-green">Save Changes</epam-button>
            </div>
            -->
          </div>
        </div>
...
<style lang="less">
  @import "../../../assets/vendors/epam-ui/less/uui-modal";
